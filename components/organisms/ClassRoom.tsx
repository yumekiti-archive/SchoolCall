                ...
                <tr className="w-1/6 h-full grid grid-row-8" key={index}>
                  {
                    row.map((seat, row_index) => (
                      calls.filter((call) => call.seat_number === seat.seat_number).map((call) => (
                        <DeskTable key={row_index} connect={index % 2 === 1} seat_number={seat.seat_number} name={call.student.name} turn_num={1} />
                      ))[0] ||
                      <DeskTable key={row_index} connect={index % 2 === 1} seat_number={seat.seat_number} name={seat.name} />
                    ))
                  }
                </tr>
              ))
