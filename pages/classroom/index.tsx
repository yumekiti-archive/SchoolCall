...
const ClassroomRegister = () => {
  ...
  return (
    <Layout Title='クラス作成'>
      <div className='w-full h-full grid grid-cols-2 grid-rows-3 gap-4 p-4'>
        {classrooms.map((classroom: any) => (
          <div key={classroom.id} className='col-span-1 row-span-1 bg-white rounded-lg shadow-lg flex flex-col justify-evenly items-center'>
            ...
            <div className='flex justify-end items-center w-8/12 rounded-lg grid grid-cols-4 gap-2'>
              ...
              <button className='col-span-1 rounded-lg p-2 bg-green-400 text-white' onClick={() => handleUpdate(classroom.id, classroom)}>
                備品
              </button>
              <button className='col-span-1 rounded-lg p-2 bg-blue-400 text-white' onClick={() => handleUpdate(classroom.id, classroom)}>
                更新
              </button>
              <button className='col-span-1 rounded-lg p-2 bg-red-400 text-white' onClick={() => handleDelete(classroom.id)}>
                削除
              </button>
            </div>
          </div>
